...
if [ -f "/usr/bin/apt-get" ];then
	...
	if [ "$isDebian" != "" ];then
		wget -O install.sh https://download.btpanel.net/install/install-ubuntu_6.0.sh && bash install.sh
		exit;
	else
		wget -O install.sh https://download.btpanel.net/install/install-ubuntu_6.0.sh && sudo bash install.sh
		exit;
	fi
fi
